...
def index():
    ...
    <body class="container py-5">
        ...
        <form action="/shorten" method="post" class="mb-3">
            ...
            <input type="text" name="your_favor_short_url" placeholder="Enter your favorite short URL" class="form-control mb-2">
...
def shorten_url():
    ...
    if existing:
        if favor_short_url:
            if favor_short_url != existing.short_code:
                return abort(409, description="Short URL already exists")
        elif existing.outdate_after.replace(tzinfo=timezone.utc) < datetime.now(timezone.utc):
            db.session.delete(existing)
            db.session.commit()
            return abort(410, description="URL expired")
        if existing.original_url.startswith("http://"):
           short_url = f"http://127.0.0.1:5000/{existing.short_code}"
        else:
            short_url = f"https://127.0.0.1:5000/{existing.short_code}"
    else:
        # 先插入占位
        ...
        if favor_short_url:
            check = URLMap.query.filter_by(short_code=favor_short_url).first()
            if check:
            ...
            if original_url.startswith("http://"):
              short_url = f"http://127.0.0.1:5000/{favor_short_url}"
            else:
              short_url = f"https://127.0.0.1:5000/{favor_short_url}"
        else:
           # 生成短码
           ...
           if original_url.startswith("http://"):
              ...
              short_url = f"https://127.0.0.1:5000/{short_code}"
